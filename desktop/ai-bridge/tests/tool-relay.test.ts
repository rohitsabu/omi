 ...
 * 2. ai-bridge receives it, forwards to Swift via stdout, creates a pending promise
 * 3. Swift executes the tool and sends {"type":"tool_result", callId, result} back via stdin
 * 4. ai-bridge's resolveToolCall() resolves the pending promise, writing back to the socket client
 *
...
describe("Tool relay: Unix socket end-to-end", () => {
  ...
  /**
   * Creates a Unix socket server that mimics the ai-bridge relay:
