...
class TaskChatState: ObservableObject {
    ...
    var onAuthRequired: AIBridge.AuthRequiredHandler?
    var onAuthSuccess: AIBridge.AuthSuccessHandler?
    ...
    private func ensureBridgeStarted() async -> Bool {
        if bridgeStarted {
            let alive = await aiBridge?.isAlive ?? false
            if !alive {
        ...
        do {
            ...
            let bridge = AIBridge(harnessMode: harness)
    ...
    func sendMessage(_ text: String, isFollowUp: Bool = false, taskContext: String? = nil) async {
        ...
        do {
            ...
            guard let bridge = aiBridge else {
